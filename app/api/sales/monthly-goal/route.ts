...
export async function GET(request: NextRequest) {
 ...
 select: { total: true, amount: true, saleDate: true },
 }),
 ])
 ...
 // Group for daily series
 const dailyMap = new Map<string, number>()
 paidSales.forEach(s => {
 if (!s.saleDate) return
 const key = s.saleDate.toISOString().slice(0, 10)
 dailyMap.set(key, (dailyMap.get(key) || 0) + (Number(s.amount ?? s.total) || 0))
 })

 const dailyRevenueSeries: Array<{ date: Date; amount: number }> = []
 const cursor = new Date(firstDay)
 const seriesEnd = now < lastDay ? now : lastDay
