...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error("[settings/webhook] Signature verification failed:", msg)
  ...
  try {
    switch (event.type) {
      ...
      case "customer.subscription.updated": {
        ...
        if (sub.status === "canceled" || sub.status === "unpaid") {
          await safePrismaQuery(() =>
            prisma.user.update({
              where: { id: userId },
              data: { plan: "STARTER", isTrial: false, stripeSubscriptionId: null, planExpiresAt: null },
            })
          )
        } else {
          await upsertSubscription(userId, sub)
      ...
      case "invoice.payment_failed": {
        ...
        console.warn(`[settings/webhook] Payment failed for user ${userId}, sub ${sub.id}`)
        break
      }
    ...
    console.error("[settings/webhook] Handler error:", errMsg)
...
async function upsertSubscription(userId: string, sub: Stripe.Subscription) {
  ...
  const billingCycle =
    (item?.price as Stripe.Price & { recurring?: { interval?: string } })?.recurring?.interval ?? null
