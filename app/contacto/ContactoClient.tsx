    ...
    >
      ...
      <section id="hero" className="h-screen flex items-center px-6 bg-[#0B1F2A]">
        <div className="mx-auto flex w-full max-w-5xl flex-col items-center text-center gap-6 reveal">
          <span className="inline-flex items-center rounded-full border border-[#E2E8ED]/30 bg-[#FFFFFF]/10 px-4 py-1 text-xs uppercase tracking-[0.3em] text-[#8FA6B2]">
            Respuesta media: &lt; 2h
          </span>
          <h1 className="text-4xl md:text-6xl font-semibold leading-tight text-[#E6F1F5]">
            Habla con el <span className="text-[#1FA97A] font-semibold">equipo</span> de ClientLabs
          </h1>
          <p className="text-[#8FA6B2] text-base md:text-lg max-w-3xl">
            Te ayudamos a diseñar tu sistema operativo.{" "}
            <span className="text-[#1FA97A] font-semibold">Soporte</span> real. Respuestas humanas.
          </p>
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <PrimaryButton href="#form" className="!bg-[#1FA97A] hover:!bg-[#157A5C] !text-white rounded-xl !border-0 !shadow-none">Contactar soporte</PrimaryButton>
            <SecondaryButton href="/demo" className="!border-[#E2E8ED] !text-[#E6F1F5] hover:!bg-white/10">Agendar demo</SecondaryButton>
          </div>
        </div>
      </section>
      <section id="form" className="h-screen flex items-center px-6 bg-[#FFFFFF]">
        <div className="mx-auto grid w-full max-w-6xl items-center gap-10 md:grid-cols-[1.05fr_0.95fr]">
          <div className="space-y-5 reveal">
            <p className="text-xs uppercase tracking-[0.3em] text-[#5F7280]">Soporte humano</p>
            <h2 className="text-3xl md:text-5xl font-semibold text-[#0F1F2A]">
              Operaciones reales requieren{" "}
              <span className="text-[#1FA97A] font-semibold">resultados</span> reales
            </h2>
            <p className="text-[#5F7280]">
              Infraestructura de <span className="text-[#1FA97A] font-semibold">soporte</span>, onboarding guiado y
              acompañamiento continuo para decisiones basadas en datos.
            </p>
            <div className="text-sm text-[#5F7280]">
              Te respondemos en menos de 2h
            </div>
          </div>
          <div className="rounded-3xl border border-[#E2E8ED] bg-[#F4F7F9] p-6 md:p-8 shadow-sm reveal">
            <form className="space-y-4" onSubmit={handleSubmit}>
              {[
                ...
                <div key={field.id} className="space-y-2">
                  <label htmlFor={field.id} className="text-sm text-[#5F7280]">
                    {field.label}
                  </label>
                  <input
                    id={field.id}
                    ...
                    className="w-full rounded-2xl border border-[#E2E8ED] bg-[#FFFFFF] px-4 py-3 text-sm text-[#0F1F2A] placeholder:text-[#5F7280]/60 focus:border-[#1FA97A] focus:outline-none"
                  />
              ...
              <div className="space-y-2">
                <label htmlFor="message" className="text-sm text-[#5F7280]">
                  Mensaje
                </label>
                <textarea
                  id="message"
              ...
              <button
                type="submit"
                className="w-full rounded-xl bg-[#1FA97A] hover:bg-[#157A5C] px-4 py-3 text-sm font-semibold text-white transition disabled:opacity-60"
              ...
              {status === "success" && (
                <p className="text-sm text-[#157A5C]">Mensaje enviado. Te respondemos en menos de 2h.</p>
              )}
              {status === "error" && (
                <p className="text-sm text-rose-500">Completa los campos para enviar tu consulta.</p>
              )}
            </form>
          </div>
        </div>
      ...
      <section id="info" className="h-screen flex items-center px-6 bg-[#F4F7F9]">
        <div className="mx-auto w-full max-w-5xl space-y-6 text-center reveal">
          <p className="text-xs uppercase tracking-[0.3em] text-[#5F7280]">Infraestructura de confianza</p>
          <h2 className="text-3xl md:text-5xl font-semibold text-[#0F1F2A]">
            <span className="text-[#1FA97A] font-semibold">Soporte</span> humano para operaciones reales
          </h2>
          <p className="text-[#5F7280] max-w-3xl mx-auto">
            Partnership real para equipos que necesitan claridad operativa, acompañamiento y{" "}
            <span className="text-[#1FA97A] font-semibold">decisiones</span> basadas en datos.
          </p>
          <div className="grid gap-4 md:grid-cols-3">
            {["Onboarding guiado", "Playbooks operativos", "Acompañamiento real"].map((item) => (
              <div key={item} className="rounded-3xl border border-[#E2E8ED] bg-[#FFFFFF] p-6 shadow-sm">
                <div className="text-sm text-[#0F1F2A]">{item}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="cta" className="h-screen flex flex-col justify-between bg-[#0B1F2A]">
        <div className="flex flex-1 items-center justify-center px-6">
          <div className="max-w-4xl text-center space-y-6 reveal">
            ...
            <p className="text-[#8FA6B2]">
              Estamos listos para darte <span className="text-[#1FA97A] font-semibold">ayuda</span> y escalar con
              control.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <PrimaryButton href="/contacto" className="!bg-[#1FA97A] hover:!bg-[#157A5C] !text-white rounded-xl !border-0 !shadow-none">Contactar ahora</PrimaryButton>
              <SecondaryButton href="/demo" className="!border-[#E2E8ED] !text-[#E6F1F5] hover:!bg-white/10">Agendar llamada</SecondaryButton>
            </div>
          </div>
        </div>
