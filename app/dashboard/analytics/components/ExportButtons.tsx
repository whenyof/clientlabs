...
export function ExportButtons({ data, onExportStart, onExportEnd }: ExportButtonsProps) {
  ...
  return (
    ...
    >
        ...
        {exportOptions.map((option) => {
          ...
          return (
            <motion.button
              ...
              disabled={isLoading}
              className={`group relative p-4 bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl transition-all duration-300 ${option.color} disabled:opacity-50 disabled:cursor-not-allowed`}
              whileHover={{ scale: 1.02, y: -2 }}
              whileTap={{ scale: 0.98 }}
            >
              <div className="flex flex-col items-center text-center space-y-3">
                <motion.div
                  className={`p-3 bg-[var(--border-subtle)] rounded-lg ${isLoading ? 'animate-pulse' : ''}`}
                ...
                >
                  {isLoading ? (
                    ...
                    <Icon className="w-6 h-6 text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]" />
                  )}
                </motion.div>
      ...
      <motion.div
        className="mt-6 p-4 bg-[var(--bg-surface)] rounded-lg border border-[var(--border-subtle)]"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <div className="text-sm text-[var(--text-secondary)]">
          <div className="flex items-center justify-between mb-2">
            <span>Reporte:</span>
            <span className="text-[var(--text-primary)] font-medium">{data.title}</span>
          </div>
          <div className="flex items-center justify-between mb-2">
            <span>Rango:</span>
            <span className="text-[var(--text-primary)] font-medium">{data.dateRange}</span>
          </div>
          <div className="flex items-center justify-between">
            <span>Registros:</span>
            <span className="text-[var(--text-primary)] font-medium">{data.summary?.totalRecords || data.tableData.length}</span>
          </div>
        </div>
      </motion.div>
    </motion.div>
