...
export function ExportPDFButton({ selectedRange, onExport }: ExportPDFButtonProps) {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(element, {
        scale: 2,
        useCORS: true,
        backgroundColor: 'var(--bg-main)', // bg-[var(--bg-card)]
        width: 1200,
        height: 1600
      })
      ...
      pdf.setFontSize(12)
      pdf.setTextColor(156, 163, 175) // text-[var(--text-secondary)]
  ...
  return (
    ...
    >
      {isExporting ? (
        <>
          <div className="w-4 h-4 border border-[var(--border-subtle)] border-t-transparent rounded-full animate-spin" />
