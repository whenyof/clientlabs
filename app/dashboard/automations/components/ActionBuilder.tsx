      ...
      <div className="space-y-4">
        {actions.map((action, index) => {
          ...
          return (
            <motion.div
              key={action.id}
              className="bg-[var(--bg-main)] border border-[var(--border-subtle)] rounded-xl p-4"
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-[var(--text-primary)] font-medium">{display.name}</h4>
                    <p className="text-[var(--text-secondary)] text-sm">Acción {index + 1}</p>
                  </div>
                </div>

                <button
                  onClick={() => removeAction(action.id)}
                  className="p-2 text-[var(--text-secondary)] hover:text-red-400 hover:bg-red-600/20 rounded-lg transition-colors"
                >
              ...
              <div className="space-y-3">
                {action.type === 'email_send' && (
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs text-[var(--text-secondary)] mb-1">Asunto</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-xs text-[var(--text-secondary)] mb-1">Retraso (horas)</label>
                      <input
                        type="number"
                ...
                {action.type === 'slack_notification' && (
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs text-[var(--text-secondary)] mb-1">Canal</label>
                      <input
                        type="text"
            ...
            {actionTypes.map((actionType) => {
              ...
              return (
                <motion.button
                  ...
                  className="flex items-start gap-3 p-4 bg-[var(--bg-main)] hover:bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl transition-colors group"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
