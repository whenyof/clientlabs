...
function StatusBadge({ status }: { status: string }) {
  const getStatusConfig = (status: string) => {
    switch (status) {
      ...
      default:
        return { color: "text-[var(--text-secondary)] bg-[var(--bg-main)]0/10", text: "Desconocido" }
    }
  }
...
export function AutomationList() {
  ...
  return (
    <div className="bg-[var(--bg-card)] backdrop-blur-xl border border-[var(--border-subtle)] rounded-xl p-6">
      <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-6">Automatizaciones Activas</h3>

      <div className="space-y-4">
        {AUTOMATIONS.map((automation) => (
          <div
            key={automation.id}
            className="bg-[var(--bg-main)] border border-[var(--border-subtle)] rounded-lg p-4 hover:bg-[var(--bg-surface)] transition-colors"
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-[var(--text-secondary)]">{automation.description}</p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
              <div>
                <p className="text-xs text-[var(--text-secondary)]">Trigger</p>
                <p className="text-sm text-[var(--text-primary)]">{automation.trigger}</p>
              </div>
              <div>
                <p className="text-xs text-[var(--text-secondary)]">Acciones</p>
                <p className="text-sm text-[var(--text-primary)]">{automation.actions.length}</p>
              </div>
              <div>
                <p className="text-xs text-[var(--text-secondary)]">Ejecuciones</p>
                <p className="text-sm text-[var(--text-primary)]">{automation.executions}</p>
              </div>
              <div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button className="p-1 text-[var(--text-secondary)] hover:text-[var(--text-secondary)]">
