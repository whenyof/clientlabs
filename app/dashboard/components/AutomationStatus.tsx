...
function getStatusIcon(status: string) {
  switch (status) {
    ...
    default:
      return <Play className="w-4 h-4 text-[var(--text-secondary)]" />
  }
}
...
function getStatusColor(status: string) {
  switch (status) {
    ...
    default:
      return "text-[var(--text-secondary)] bg-[var(--bg-main)]0/10"
  }
}
    ...
    <div className="bg-[var(--bg-card)] backdrop-blur-xl border border-[var(--border-subtle)] rounded-xl p-6">
      ...
      <div className="space-y-3">
        {AUTOMATIONS.map((automation) => (
          <div key={automation.id} className="flex items-center justify-between p-3 bg-[var(--bg-main)] rounded-lg">
            <div className="flex items-center gap-3">
              {getStatusIcon(automation.status)}
              <div>
                <p className="font-medium text-[var(--text-primary)] text-sm">{automation.name}</p>
                <p className="text-[var(--text-secondary)] text-xs">{automation.lastRun}</p>
              </div>
            </div>
            <div className="text-right">
              ...
              <p className="text-[var(--text-secondary)] text-xs mt-1">{automation.executions} ejecuciones</p>
            </div>
          </div>
        ))}
      </div>
