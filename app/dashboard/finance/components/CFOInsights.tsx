...
export function CFOInsights() {
  ...
  if (loading) {
    return (
      <section aria-label="Insights CFO" className="w-full">
        <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)]/[0.03] p-6 animate-pulse h-28" />
      </section>
    )
  }
  ...
  if (!hasEnoughData || insights.length === 0) {
    return (
      <section aria-label="Insights CFO" className="w-full">
        <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)]/[0.03] p-8 text-center">
          <p className="text-sm text-[var(--text-secondary)]">{EMPTY_MESSAGE}</p>
        </div>
      </section>
    )
  }
