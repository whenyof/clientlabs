...
function getStatusColor(s: string) {
  return s === 'connected' ? 'bg-green-500/20 text-green-400' : s === 'error' ? 'bg-red-500/20 text-red-400' : s === 'pending' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-[var(--bg-main)]0/20 text-[var(--text-secondary)]'
}
        ...
        return (
          <>
            ...
            <motion.button
              ...
              className="flex items-center gap-2 px-3 py-2 bg-[var(--bg-main)]0/20 hover:bg-[var(--bg-main)]0/30 text-[var(--text-secondary)] hover:text-[var(--text-secondary)] rounded-lg transition-colors text-sm"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
      ...
      case 'disconnected':
        return (
          <motion.button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-[var(--text-primary)] rounded-lg transition-colors text-sm font-medium"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          {filteredIntegrations.length === 0 && (
            <p className="text-[var(--text-secondary)] text-sm mt-2">Conecta integraciones desde el panel para verlas aquí.</p>
          )}
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="mb-4">
                <div className="flex flex-wrap gap-1">
                  ...
                  {integration.features.length > 2 && (
                    <span className="px-2 py-1 bg-[var(--bg-main)]0/10 text-[var(--text-secondary)] text-xs rounded-full">
              ...
              {integration.status === 'connected' && (
                <div className="grid grid-cols-2 gap-4 mb-4 p-3 bg-[var(--bg-card)] rounded-lg">
                  <div>
              ...
              {integration.status === 'error' && integration.errorMessage && (
                <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                  ...
                  <div className="text-xs text-[var(--text-secondary)]">{integration.errorMessage}</div>
                </div>
              )}
