...
export function AISuggestionCard({ leadId, suggestion, onApplyAction }: AISuggestionCardProps) {
    ...
    return (
        <div className={`rounded-xl border-l-4 ${colors.border} ${colors.bg} border border-[var(--border-subtle)] p-4 space-y-3`}>
            {/* Header */}
            <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2">
                    <Sparkles className={`h-5 w-5 ${colors.text}`} />
                    <div>
                        <h3 className="text-[var(--text-primary)] font-semibold text-sm">Sugerencia inteligente</h3>
            ...
            <div className="pl-7 flex items-center gap-2">
                ...
                <span className="text-xs text-[var(--text-secondary)]">Acción recomendada</span>
            </div>
        </div>
    )
}
...
export function AISuggestionLoading() {
    return (
        <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-main)] p-4 space-y-3">
            <div className="flex items-center gap-2">
                <Loader2 className="h-5 w-5 text-blue-400 animate-spin" />
                <div>
                    <h3 className="text-[var(--text-primary)] font-semibold text-sm">Analizando lead...</h3>
                    <p className="text-xs text-[var(--text-secondary)]">Calculando mejor acción</p>
                </div>
            </div>
        </div>
    )
    ...
    return (
        <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-main)] p-4">
            <div className="flex items-center gap-2">
                ...
                <div>
                    <h3 className="text-[var(--text-primary)] font-semibold text-sm">Sin sugerencias</h3>
                    <p className="text-xs text-[var(--text-secondary)]">No hay acciones recomendadas en este momento</p>
                </div>
            </div>
        </div>
    )
