...
export function AutomationsPanel({ open, onClose, selectedLead }: AutomationsPanelProps) {
    ...
    return (
        <>
            {/* Overlay */}
            <div
                className="fixed inset-0 bg-[var(--bg-card)]/40 backdrop-blur-sm z-40"
                onClick={onClose}
            />

            {/* Panel */}
            <div className="fixed right-0 top-0 h-full w-full max-w-2xl bg-[var(--bg-card)] border-l border-[var(--border-subtle)] shadow-2xl z-50 overflow-hidden flex flex-col">
                {/* Header */}
                    ...
                    {selectedLead && (
                        <div>
                            ...
                            {loading ? (
                                ...
                                <div className="p-6 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-main)] text-center">
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {SYSTEM_AUTOMATIONS.map((automation) => (
                                <div
                                    key={automation.id}
                                    className="p-4 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-main)] hover:bg-[var(--bg-surface)] transition-colors"
                                >
