...
export function ImportHistoryPanel({ batches }: ImportHistoryPanelProps) {
    ...
    return (
        <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-main)]">
            {/* Collapsed Header */}
            <button
                onClick={() => setIsExpanded(!isExpanded)}
                className="w-full flex items-center justify-between p-4 hover:bg-[var(--bg-main)] transition-colors"
            >
                    ...
                    {batches.map((batch, idx) => (
                        <div
                            ...
                            className={`rounded-lg border p-3 ${idx === 0
                                    ...
                                    : "border-[var(--border-subtle)] bg-[var(--bg-main)] hover:bg-[var(--bg-surface)]"
                                } transition-all`}
                        >
                            <div className="flex items-center justify-between gap-4">
                                <div className="flex items-center gap-3 flex-1">
                                    <div className={`p-2 rounded-lg ${idx === 0
                                        ...
                                        {batch.type === "csv" ? (
                                            <FileText className={`h-4 w-4 ${idx === 0 ? "text-blue-400" : "text-[var(--text-secondary)]"}`} />
                                        ) : batch.type === "excel" ? (
                                            <FileSpreadsheet className={`h-4 w-4 ${idx === 0 ? "text-blue-400" : "text-[var(--text-secondary)]"}`} />
                                        ) : (
                                            <Upload className={`h-4 w-4 ${idx === 0 ? "text-blue-400" : "text-[var(--text-secondary)]"}`} />
                                        )}
                                    </div>
                                ...
                                <Button
                                    ...
                                    className={idx === 0
                                        ...
                                        : "bg-[var(--bg-main)] border-[var(--border-subtle)] text-[var(--text-secondary)] hover:bg-[var(--bg-surface)]"
                                    }
                                >
