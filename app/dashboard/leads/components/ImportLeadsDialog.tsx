...
export function ImportLeadsDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
    ...
    return (
        <Dialog open={open} onOpenChange={(open) => {
            ...
            <DialogContent className="bg-[var(--bg-card)] border-[var(--border-subtle)] max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
                <DialogHeader>
                                        ...
                                        <tbody>
                                            {leads.map((lead, idx) => (
                                                <tr key={idx} className={`border-t border-[var(--border-subtle)] hover:bg-[var(--bg-main)] transition-all ${lead.excluded ? 'opacity-40' : ''
                                                    ...
                                                    <td className={`px-3 py-2 ${lead.excluded ? 'text-[var(--text-secondary)]' : 'text-[var(--text-primary)]'}`}>{lead.name || "-"}</td>
                                                    <td className={`px-3 py-2 ${lead.excluded ? 'text-[var(--text-secondary)]' : 'text-[var(--text-secondary)]'}`}>{lead.email || "-"}</td>
                                                    <td className="px-3 py-2">
                                                        {lead.tags && lead.tags.length > 0 ? (
                                                            <div className="flex flex-wrap gap-1">
                                                                {lead.tags.slice(0, 3).map((tag, tagIdx) => {
                                                                    let tagColor = "bg-[var(--bg-main)]0/20 text-[var(--text-secondary)]"
