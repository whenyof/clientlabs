  ...
  return (
    <div className={`
      ...
      ${notification.read
        ...
        : `bg-[var(--bg-card)] border-[var(--border-subtle)] ${colorMap[notification.type]}`
      }
    `}>
      <div className="flex items-start gap-3">
        <div className={`p-2 rounded-lg ${
          notification.read ? 'bg-[var(--bg-surface)]' : colorMap[notification.type]
        }`}>
          <Icon className={`w-4 h-4 ${
            notification.read ? 'text-[var(--text-secondary)]' : ''
          }`} />
        </div>
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className={`text-sm mt-1 ${
                notification.read ? 'text-[var(--text-secondary)]' : 'text-[var(--text-secondary)]'
              }`}>
