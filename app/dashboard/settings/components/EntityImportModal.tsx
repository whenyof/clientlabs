...
export function EntityImportModal({
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const data: ImportResult & { error?: string } = await res.json().catch(() => ({ error: "Error inesperado" } as ImportResult & { error?: string }))
      if (!res.ok) throw new Error(data.error ?? "Error al importar")
      setResult(data)
      if (data.imported + data.updated > 0) {
        toast.success(`${data.imported} importados · ${data.updated} actualizados`)
        onDone()
      }
    } catch (err) {
        ...
        <div className="p-6 space-y-5">
          ...
          <div
            ...
            className={cn(
              ...
              dragging ? "border-[#0F766E] bg-[#E1F5EE]/40"
                : file ? "border-[#0F766E]/40 bg-[#E1F5EE]/20"
          ...
          {/* Preview */}
          {preview.length > 0 && previewHeaders.length > 0 && (
              ...
              {result.errors.length > 0 && (
                <ul className="mt-2 space-y-1">
                  {result.errors.map((e, i) => (
                    <li key={i} className="text-[11px] text-red-500">Fila {e.row}: {e.message}</li>
                  ))}
                </ul>
              )}
            </div>
