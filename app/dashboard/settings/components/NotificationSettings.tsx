...
export function NotificationSettings() {
  ...
  return (
    ...
    >
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-2">Notificaciones</h2>
        <p className="text-[var(--text-secondary)]">Configura cómo quieres recibir las notificaciones</p>
      </div>

      <div className="space-y-8">
        {/* Marketing Preferences */}
        <div className="bg-[var(--bg-card)] rounded-xl p-6">
          <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-4">Preferencias de marketing</h3>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-[var(--text-primary)] font-medium">Emails de marketing</div>
                <div className="text-sm text-[var(--text-secondary)]">Recibe ofertas especiales y actualizaciones del producto</div>
              </div>
              <motion.button
                onClick={() => setMarketingEmails(!marketingEmails)}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  marketingEmails ? 'bg-purple-600' : 'bg-[var(--bg-surface)]'
                }`}
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="text-[var(--text-primary)] font-medium">Resumen semanal</div>
                <div className="text-sm text-[var(--text-secondary)]">Recibe un resumen semanal de tu actividad</div>
              </div>
              <motion.button
                onClick={() => setWeeklyDigest(!weeklyDigest)}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  weeklyDigest ? 'bg-purple-600' : 'bg-[var(--bg-surface)]'
                }`}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
            ...
            >
              ...
              <div className="space-y-4">
                {group.items.map((item) => {
                  ...
                  return (
                    <div key={item.key} className="flex items-center justify-between py-3 border-b border-[var(--border-subtle)] last:border-b-0">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            className="w-4 h-4 text-purple-600 bg-[var(--bg-surface)] border-[var(--border-subtle)] rounded focus:ring-purple-500"
                          />
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <ChatBubbleLeftIcon className="w-4 h-4 text-[var(--text-secondary)]" />
                          <input
                            type="checkbox"
