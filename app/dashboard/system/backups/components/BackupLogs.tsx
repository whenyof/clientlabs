...
export function BackupLogs({ logs }: BackupLogsProps) {
  ...
  const getLevelBg = (level: string) => {
    switch (level.toLowerCase()) {
      ...
      case 'debug':
        return 'bg-[var(--bg-main)]0/10 border-[var(--border-subtle)]'
      default:
        return 'bg-[var(--bg-main)] border-[var(--border-subtle)]'
    }
  }
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-[var(--text-primary)]">Logs del Sistema</h3>
      ...
      {logs.length === 0 ? (
        <div className="text-center py-16">
          <DocumentTextIcon className="w-16 h-16 text-[var(--text-primary)] mx-auto mb-4" />
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {formatted.data && Object.keys(formatted.data).length > 2 && (
                      <details className="mt-2">
                        <summary className="text-xs text-[var(--text-secondary)] cursor-pointer hover:text-[var(--text-secondary)]">
                          Mostrar detalles
                        </summary>
                        <pre className="mt-2 text-xs text-[var(--text-secondary)] bg-[var(--bg-card)] p-2 rounded overflow-x-auto">
