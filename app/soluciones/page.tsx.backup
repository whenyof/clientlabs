...
export default function Soluciones() {
  ...
  return (
    <main className="relative min-h-screen bg-[#FFFFFF]">
      ...
      <section className="snap-section min-h-screen bg-[#0B1F2A]" data-section={0}>
        <div className="mx-auto flex h-full w-full max-w-4xl flex-col items-center justify-center px-6 text-center space-y-6">
          ...
          <p className="text-[#8FA6B2] text-base md:text-lg">
            ...
            <span className="text-[#1FA97A] font-semibold">control</span> operativo.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <PrimaryButton href="/contacto" className="!bg-[#1FA97A] hover:!bg-[#157A5C] !text-white rounded-xl !border-0 !shadow-none">Probar gratis 14 días</PrimaryButton>
            <SecondaryButton href="/producto" className="!border-[#E2E8ED] !text-[#E6F1F5] hover:!bg-white/10">Ver cómo funciona</SecondaryButton>
          </div>
        </div>
      </section>

      <section className="snap-section min-h-screen bg-[#FFFFFF]" data-section={1}>
        <div className="mx-auto grid h-full w-full max-w-6xl items-center gap-12 px-6 md:grid-cols-[1.05fr_0.95fr]">
          <div className="space-y-5 animate-on-view">
            ...
            <h2 className="text-3xl md:text-5xl font-semibold leading-tight text-[#0F1F2A]">
              El problema no es crecer. <br /> Es perder el{" "}
              <span className="text-[#1FA97A] font-semibold">control</span> cuando creces.
            </h2>
            <p className="text-[#5F7280] text-base md:text-lg">
              Empresas con buen producto fallan por{" "}
              <span className="text-[#1FA97A] font-semibold">sistemas</span> débiles: datos dispersos, equipos
              desalineados y decisiones a ciegas. ClientLabs nace para resolver exactamente eso.
            </p>
            <div className="grid gap-3 text-[#0F1F2A]">
          ...
          <div className="relative animate-on-view">
            <div className="relative rounded-3xl border border-[#E2E8ED] bg-[#F4F7F9] p-6 shadow-sm">
              <div className="flex items-center justify-between border-b border-[#E2E8ED] pb-4 text-xs uppercase tracking-[0.3em] text-[#5F7280]">
                <span>Control</span>
                <span>Live</span>
              </div>
              <div className="mt-6 grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={label} className="rounded-2xl border border-[#E2E8ED] bg-[#FFFFFF] p-4">
                    <p className="text-xs text-[#5F7280]">{label}</p>
                    <p className="mt-2 text-lg font-semibold text-[#0F1F2A]">{value}</p>
                  </div>
                ))}
              </div>
              <div className="mt-6 rounded-2xl border border-[#E2E8ED] bg-[#FFFFFF] p-4">
                <p className="text-xs text-[#5F7280]">Rendimiento operativo</p>
                <div className="mt-3 h-2 rounded-full bg-[#E2E8ED] overflow-hidden">
      ...
      <section className="snap-section min-h-screen bg-[#F4F7F9]" data-section={2}>
        <div className="mx-auto flex h-full w-full max-w-6xl flex-col justify-center gap-10 px-6">
          <div className="space-y-4 animate-on-view">
            <p className="text-xs uppercase tracking-[0.3em] text-[#5F7280]">
              El <span className="text-[#1FA97A] font-semibold">crecimiento</span> no es el problema.
            </p>
            <h2 className="text-3xl md:text-5xl font-semibold leading-tight text-[#0F1F2A]">
              El problema es perder el <span className="text-[#1FA97A] font-semibold">control</span> cuando creces.
            </h2>
            <p className="text-[#5F7280] text-base md:text-lg max-w-3xl">
          ...
          <div className="grid gap-8 md:grid-cols-3 text-[#5F7280]">
            {[
              {
                title: "Desorden",
              ...
              <div key={col.title} className="animate-on-view stagger-item">
                <div className="text-xs uppercase tracking-[0.3em] text-[#157A5C]">{col.title}</div>
                <div className="mt-3 text-lg md:text-xl font-semibold text-[#0F1F2A]">{col.subtitle}</div>
                ...
                {index < 2 && <div className="mt-8 h-px w-full bg-[#E2E8ED] md:hidden" />}
              </div>
            ))}
          </div>

          <div className="mx-auto max-w-[680px] text-center mt-8 md:mt-14 animate-on-view delayed-emphasis">
            <p className="text-xl md:text-3xl font-semibold text-[#0F1F2A]">
              El problema no es tu negocio.
            </p>
            <p className="mt-2 text-base md:text-2xl text-[#5F7280]">
              Es el <span className="text-[#1FA97A]">sistema</span> que lo sostiene.
            </p>
          </div>
        </div>
      </section>

      <section className="snap-section min-h-screen bg-[#FFFFFF]" data-section={3}>
        <div className="mx-auto flex h-full w-full max-w-5xl flex-col items-center justify-center px-6 text-center space-y-6">
            <h2 className="text-3xl md:text-5xl font-semibold text-[#0F1F2A]">
              No trabajamos con plantillas. Diseñamos{" "}
              <span className="text-[#1FA97A] font-semibold">sistemas</span> según tu negocio.
            </h2>
          <div className="mx-auto grid gap-4 text-[#5F7280] md:grid-cols-2">
            {[
              ...
              <div key={item} className="flex items-center gap-3 justify-center md:justify-start">
                <span className="h-1.5 w-1.5 rounded-full bg-[#1FA97A]" />
                <span>{item}</span>
              </div>
            ))}
          </div>
      ...
      <section className="snap-section min-h-screen bg-[#F4F7F9]" data-section={4}>
        <div className="mx-auto grid h-full w-full max-w-6xl items-center gap-10 px-6 md:grid-cols-[1fr_1.1fr]">
          <div className="space-y-5">
            ...
            <h2 className="text-3xl md:text-4xl font-semibold text-[#0F1F2A]">NextSite</h2>
            <p className="text-[#5F7280]">
              Reducción del 63% en tiempo operativo. Recuperación automática de pagos.{" "}
              <span className="text-[#1FA97A] font-semibold">Control</span> total en 30 días.
            </p>
          </div>
          <div className="rounded-3xl border border-[#E2E8ED] bg-[#FFFFFF] p-6 shadow-sm">
            <div className="flex items-center justify-between border-b border-[#E2E8ED] pb-4 text-xs uppercase tracking-[0.3em] text-[#5F7280]">
              <span>Panel</span>
              <span>Live</span>
            </div>
            <div className="mt-5 grid grid-cols-2 gap-4">
              {[
                ...
                <div key={label} className="rounded-2xl border border-[#E2E8ED] bg-[#F4F7F9] p-4">
                  <p className="text-xs text-[#5F7280]">{label}</p>
                  <p className="mt-2 text-lg font-semibold text-[#0F1F2A]">{value}</p>
                </div>
              ))}
            </div>
            <div className="mt-5 rounded-2xl border border-[#E2E8ED] bg-[#F4F7F9] p-4">
              <p className="text-xs text-[#5F7280]">Automatizaciones activas</p>
              <div className="mt-3 h-2 rounded-full bg-[#E2E8ED] overflow-hidden">
                <div className="h-full w-4/5 rounded-full bg-[#1FA97A]" />
              </div>
            </div>
      ...
      <section className="snap-section min-h-screen bg-[#FFFFFF]" data-section={5}>
        <div className="mx-auto flex h-full w-full max-w-4xl flex-col items-center justify-center px-6 text-center space-y-5">
          ...
          <p className="text-[#5F7280] text-base md:text-lg">
            Arquitectura <span className="text-[#1FA97A] font-semibold">escalable</span> para crecer sin romper nada.
          </p>
        </div>
      </section>

      <section className="snap-section min-h-screen bg-[#0B1F2A]" data-section={6}>
        <div className="flex h-full w-full flex-col justify-between">
          <div className="flex flex-1 items-center justify-center px-6">
            <div className="max-w-4xl text-center space-y-6">
              <h2 className="text-4xl md:text-5xl font-semibold text-[#E6F1F5]">
                Deja de improvisar. Opera con{" "}
                <span className="text-[#1FA97A] font-semibold">control</span> como una empresa seria.
              </h2>
              <PrimaryButton href="/contacto" className="!bg-[#1FA97A] hover:!bg-[#157A5C] !text-white rounded-xl !border-0 !shadow-none">Probar gratis 14 días</PrimaryButton>
            </div>
          </div>
