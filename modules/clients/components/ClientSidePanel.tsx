...
export function ClientSidePanel({ client, isOpen, onClose, onClientUpdate }: ClientSidePanelProps) {
 ...
 toast.success(labels.tasks.singular + " creada")
 ...
 id: newSale.id,
 type: "SALE",
 title: labels.sales.singular + " registrada",
 ...
 toast.success(labels.providers.actions.addNote + " " + labels.common.success.toLowerCase())
 setTimeout(() => {
 loadTimeline().then(() => {
 ...
 toast.success(labels.sales.singular + " registrada correctamente")
 setTimeout(() => {
 loadTimeline().then(() => {
 setOptimisticEvents(prev => prev.filter(e => e.id !== tempId))
 })
 ...
 className={`fixed inset-y-0 right-0 w-full md:w-[600px] bg-zinc-950 border-l border-[var(--border-subtle)] shadow-sm z-50 ${isOpen ? "pointer-events-auto" : "pointer-events-none"}`}
 >
 <div className="h-full flex flex-col overflow-hidden bg-zinc-950">
 <div className="p-6 border-b border-[var(--border-subtle)] bg-zinc-950 shrink-0">
 <div className="flex items-start justify-between mb-4">
 <div className="flex-1 min-w-0">
 <div className="flex flex-wrap items-center gap-3 mb-3">
 <h2 className="text-xl font-bold text-[var(--text-primary)] tracking-tight truncate">{client.name || labels.common.noResults}</h2>
 ...
 <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-[0.2em] leading-none">Perfil</p>
 </div>
 <TraitsSelector
 ...
 {labels.clients.singular} desde {isMounted ? formatDistanceToNow(client.createdAt, { addSuffix: true, locale: es }) : ""}
 </p>
 <div className="flex flex-wrap gap-4">
 {client.email && (
 <a href={`mailto:${client.email}`} className="text-sm text-[var(--text-secondary)] hover:text-[var(--accent)] transition-colors flex items-center gap-2 group">
 <div className="p-1.5 rounded-md bg-[var(--bg-card)] group-hover:bg-[var(--bg-card)] transition-colors">
 ...
 {client.phone && (
 <a href={`tel:${client.phone}`} className="text-sm text-[var(--text-secondary)] hover:text-green-400 transition-colors flex items-center gap-2 group">
 <div className="p-1.5 rounded-md bg-[var(--bg-card)] group-hover:bg-green-500/10 transition-colors">
 ...
 <Button variant="ghost" size="icon" onClick={onClose} className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-card)] shrink-0">
 <X className="h-5 w-5" />
 </Button>
 </div>
 <div className="flex flex-wrap gap-2">
 <Button onClick={() => setNoteDialog(true)} className="flex-1 bg-[var(--bg-card)] border border-[var(--border-subtle)] hover:bg-[var(--bg-card)] h-11 rounded-xl">
 <MessageSquare className="h-4 w-4 mr-2 text-[var(--accent)]" /> {labels.providers.actions.addNote}
 </Button>
 <Button onClick={() => setReminderDialog(true)} className="flex-1 bg-[var(--bg-card)] border border-[var(--border-subtle)] hover:bg-[var(--bg-card)] h-11 rounded-xl">
 <Bell className="h-4 w-4 mr-2 text-[var(--text-secondary)]" /> Recordatorio
 </Button>
 <Button onClick={() => setCallDialog(true)} className="flex-1 bg-[var(--bg-card)] border border-[var(--border-subtle)] hover:bg-[var(--bg-card)] h-11 rounded-xl">
 <Phone className="h-4 w-4 mr-2 text-green-400" /> {labels.leads.actions?.registerCall || "Llamada"}
 </Button>
 <Button onClick={() => setSaleDialog(true)} className="flex-1 bg-[var(--accent-soft)] hover:bg-[var(--accent-soft)] text-[var(--text-primary)] h-11 rounded-xl border-none">
 <DollarSign className="h-4 w-4 mr-2" /> {labels.sales.singular}
 </Button>
 </div>
 </div>

 <div className="flex-1 overflow-y-auto custom-scrollbar bg-zinc-950 pb-20">
 <div className="border-b border-[var(--border-subtle)] px-6 bg-zinc-950 sticky top-0 z-20">
 <div className="flex gap-4">
 <button
 onClick={() => setActiveTab("timeline")}
 className={cn(
 "px-4 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2",
 activeTab === "timeline" ? "border-blue-500 text-[var(--text-primary)]" : "border-transparent text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
 )}
 >
 ...
 className={cn(
 "px-4 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2",
 activeTab === "tasks" ? "border-blue-500 text-[var(--text-primary)]" : "border-transparent text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
 )}
 >
 ...
 className={cn(
 "px-4 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2",
 activeTab === "sales" ? "border-blue-500 text-[var(--text-primary)]" : "border-transparent text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
 )}
 >
 ...
 className={cn(
 "px-4 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2",
 activeTab === "notes" ? "border-blue-500 text-[var(--text-primary)]" : "border-transparent text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
 )}
 >
 ...
 <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-wider">Estado</p>
 <div className="flex items-center">
 {client.status === "ACTIVE" && <span className="text-sm font-bold text-[var(--accent)]">{labels.clients.status.ACTIVE}</span>}
 {client.status === "FOLLOW_UP" && <span className="text-sm font-bold text-[var(--accent)]">{labels.clients.status.FOLLOW_UP}</span>}
 {client.status === "INACTIVE" && <span className="text-sm font-bold text-zinc-500">{labels.clients.status.INACTIVE}</span>}
 {client.status === "VIP" && <span className="text-sm font-bold text-[var(--text-secondary)]">{labels.clients.status.VIP}</span>}
 </div>
 </div>
 <div className="space-y-1">
 <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-wider">Total Facturado</p>
 ...
 <div className="space-y-1">
 <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-wider">{labels.sales.plural}</p>
 <p className="text-sm font-bold text-[var(--text-primary)]">
 {sales.length} <span className="text-[var(--text-secondary)] font-normal">items</span>
 </p>
 </div>
 </div>
 </div>
 ...
 {isForgottenBadge && (
 <div className="m-8 bg-[var(--bg-card)] border border-[var(--border-subtle)] p-4 rounded-2xl">
 <div className="flex items-start gap-4">
 <AlertTriangle className="h-5 w-5 text-[var(--text-secondary)]" />
 <div>
 <h4 className="text-sm font-bold text-[var(--text-secondary)]">Sin seguimiento activo</h4>
 <p className="text-xs text-[var(--text-secondary)] mt-1">No hay actividad desde hace {daysSinceActivityBadge} días.</p>
 </div>
 </div>
 </div>
 )}
 ...
 task.status === "DONE" ? "line-through text-[var(--text-secondary)]" : "text-[var(--text-primary)]"
 )}>
 {task.title}
 </h4>
 <div className="flex items-center gap-2 mt-0.5">
 {task.dueDate && (
 <p className="text-[10px] text-[var(--text-secondary)]">Vence: {format(new Date(task.dueDate), "d MMM", { locale: es })}</p>
 )}
 ...
 <h3 className="text-xs font-bold text-[var(--text-secondary)] uppercase tracking-[0.2em]">Historial de {labels.sales.plural}</h3>
 </div>
 <Button onClick={() => setSaleDialog(true)} size="sm" className="bg-[var(--accent-soft)] text-[var(--text-primary)] rounded-xl">
 <Plus className="h-3.5 w-3.5 mr-2" /> {labels.sales.singular}
 </Button>
 </div>
 <div className="space-y-3">
 {sales.map(sale => (
 <div key={sale.id} className="p-4 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-subtle)] cursor-pointer" onClick={() => handleSaleClick(sale)}>
 <div className="flex justify-between items-start">
 <div>
 <p className="font-bold text-[var(--text-primary)] text-sm">{sale.product}</p>
 <p className="text-[10px] text-[var(--text-secondary)] mt-1">{isMounted ? formatDate(new Date(sale.saleDate), "dd MMM yyyy", { locale: es }) : ""}</p>
 </div>
 <p className="font-bold text-[var(--text-primary)] text-sm">{formatCurrency(sale.total, sale.currency || 'EUR')}</p>
 </div>
 ...
 {sales.length === 0 && (
 <div className="text-center py-8 border border-dashed border-[var(--border-subtle)] rounded-2xl">
 <p className="text-xs text-[var(--text-secondary)]">No hay registros</p>
 </div>
 )}
 </div>
 ...
 <h3 className="text-xs font-bold text-[var(--text-secondary)] uppercase tracking-[0.2em]">Facturas relacionadas</h3>
 </div>
 {loadingInvoices ? (
 <p className="text-xs text-[var(--text-secondary)]">Cargando…</p>
 ) : clientInvoices.length === 0 ? (
 <p className="text-xs text-[var(--text-secondary)]">Ninguna factura vinculada a este cliente</p>
 ) : (
 <div className="space-y-2">
 ...
 className="flex justify-between items-center p-3 rounded-xl bg-[var(--bg-card)] border border-[var(--border-subtle)] hover:bg-[var(--bg-card)] transition-colors"
 >
 <span className="text-sm font-medium text-[var(--text-primary)]">{inv.number}</span>
 <span className="text-sm text-[var(--text-secondary)]">{formatCurrency(inv.total, inv.currency || "EUR")}</span>
 ...
 <h3 className="text-xs font-bold text-[var(--text-secondary)] uppercase tracking-[0.2em]">Notas</h3>
 </div>
 ...
 className="bg-[var(--bg-card)] border-[var(--border-subtle)] text-[var(--text-primary)] min-h-[100px] rounded-2xl"
 />
 <div className="flex justify-end">
 <Button onClick={handleAddNote} disabled={!note.trim()} className="bg-[var(--bg-card)] text-black rounded-xl">Guardar Nota</Button>
 </div>
 </div>
 </div>
 )}
 ...
 <DialogHeader><DialogTitle className="text-[var(--text-primary)]">Añadir Nota Rápida</DialogTitle></DialogHeader>
 <div className="py-4">
 <Textarea
 value={quickNote}
 ...
 <DialogFooter>
 <Button variant="outline" onClick={() => setNoteDialog(false)} className="bg-[var(--bg-card)] border-[var(--border-subtle)] text-[var(--text-primary)]">Cancelar</Button>
