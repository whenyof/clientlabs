...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
 ...
 CANCELED: { label: "Cancelado", color: "bg-[var(--bg-card)] text-[var(--critical)] border-[var(--critical)]" },
...
export function SalesList({ sales, onEdit, onUpdate }: SalesListProps) {
 ...
 if (sales.length === 0) {
 return (
 <div className="text-center py-8 border border-[var(--border-subtle)] rounded-lg bg-[var(--bg-card)]">
 <ShoppingCart className="h-12 w-12 mx-auto text-[var(--text-secondary)] mb-2" />
 ...
 <span className="text-sm text-[var(--text-secondary)]">Total gastado</span>
 ...
 <div
 key={sale.id}
 className="flex items-center justify-between p-3 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)] hover:bg-[var(--bg-card)] transition-colors"
 >
 ...
 variant="ghost"
 size="sm"
 className="h-8 w-8 p-0 text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-card)]"
 ...
 <DropdownMenuItem
 onClick={() => onEdit(sale)}
 className="text-[var(--text-primary)] hover:bg-[var(--bg-card)] cursor-pointer"
 >
 ...
 className="text-green-400 hover:bg-[var(--bg-card)] cursor-pointer"
 >
