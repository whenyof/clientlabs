...
const STATUS_STYLES: Record<string, string> = {
 ...
 CANCELED: "bg-[var(--bg-card)] text-[var(--critical)] border-[var(--critical)]",
}
...
export function InvoiceDrawer({
 ...
 aria-hidden
 className={cn(
 "fixed inset-0 z-40 bg-[var(--bg-card)] transition-opacity",
 ...
 "bg-[var(--bg-main)] border-l border-[var(--border-subtle)] shadow-sm flex flex-col",
 ...
 type="button"
 onClick={onClose}
 className="p-2 rounded-lg text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-card)] transition-colors shrink-0"
 aria-label="Cerrar"
 >
 ...
 {isDraft && (
 <div className="px-5 py-2 bg-[var(--bg-card)] border-b border-[var(--border-subtle)] shrink-0">
 <p className="text-xs text-[var(--text-secondary)]">Esta factura aún no tiene número definitivo.</p>
 </div>
 )}

 {!editable && (
 <div className="px-5 py-2 bg-zinc-600/20 border-b border-[var(--border-subtle)] shrink-0" title={ISSUED_EDIT_TOOLTIP}>
 ...
 {/* Quick actions */}
 <div className="px-5 py-2 border-b border-[var(--border-subtle)] flex flex-wrap gap-2 shrink-0">
 {!isDraft && onEditBlocked && (
 <button
 type="button"
 onClick={onEditBlocked}
 className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)] px-3 py-1.5 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-card)] hover:text-[var(--text-secondary)] transition-colors"
 title={ISSUED_EDIT_TOOLTIP}
 >
 ...
 className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)] px-3 py-1.5 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-card)] transition-colors"
 ...
 className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)] px-3 py-1.5 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-card)] disabled:opacity-50"
 >
 ...
 disabled={actionLoading}
 className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--critical)] bg-[var(--bg-card)] px-3 py-1.5 text-sm text-[var(--critical)] hover:bg-[var(--bg-card)] disabled:opacity-50"
 >
 ...
 className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--accent)] bg-[var(--accent-soft)] px-3 py-1.5 text-sm text-[var(--accent)] hover:bg-[var(--accent-soft)] disabled:opacity-50"
 >
 ...
 type="button"
 disabled
 className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)] px-3 py-1.5 text-sm text-[var(--text-secondary)] cursor-not-allowed"
 ...
 {/* Cliente */}
 <div>
 <p className="text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider mb-1">Cliente</p>
 <div className="flex flex-wrap items-center gap-2">
 <p className="text-sm text-[var(--text-secondary)] font-medium">{invoice.Client?.name || "—"}</p>
 <InvoiceClientRiskBadge clientId={invoice.clientId} invoiceType={invoice.type} />
 </div>
 {invoice.Client?.email && (
 <p className="text-sm text-[var(--text-secondary)]">{invoice.Client.email}</p>
 )}
 </div>

 {/* Líneas */}
 <div>
 <p className="text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider mb-2">Líneas</p>
 <div className="rounded-lg border border-[var(--border-subtle)] overflow-hidden">
 <table className="w-full text-sm">
 <thead>
 <tr className="bg-[var(--bg-card)]/[0.03] border-b border-[var(--border-subtle)]">
 <th className="text-left py-2 px-3 text-[var(--text-secondary)] font-medium">Descripción</th>
 ...
 <td className="py-2 px-3 text-[var(--text-secondary)]">{line.description}</td>
 <td className="py-2 px-3 text-right text-[var(--text-secondary)]">{line.quantity}</td>
 ...
 {/* Totales */}
 <div>
 <p className="text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider mb-1">Totales</p>
 ...
 <div className="flex justify-between text-[var(--text-primary)] font-medium pt-1 border-t border-[var(--border-subtle)]">
 <span>Total</span>
 ...
 <p className="text-sm font-medium text-[var(--text-secondary)]">Registrar pago</p>
 <input
 type="number"
 step="0.01"
 placeholder="Importe"
 ...
 className="w-full rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)] px-3 py-2 text-sm text-[var(--text-primary)] placeholder-white/40"
 />
 <select
 value={paymentMethod}
 ...
 className="rounded-lg bg-[var(--accent-soft)] text-[var(--accent)] px-3 py-2 text-sm font-medium hover:bg-[var(--accent-soft)] disabled:opacity-50"
 >
 Registrar
 </button>
 <button
 ...
 className="rounded-lg border border-[var(--border-subtle)] px-3 py-2 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-card)]"
 >
 Cancelar
 </button>
 </div>
 ...
 <div>
 <p className="text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider mb-2">Pagos</p>
 <ul className="space-y-2">
 {invoice.payments.map((p) => (
 <li
 key={p.id}
 className="flex justify-between items-center rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)]/[0.02] px-3 py-2 text-sm"
 >
 <span className="text-[var(--text-secondary)]">{formatCurrency(p.amount, invoice.currency)}</span>
 <span className="text-[var(--text-secondary)] text-xs">{formatDateTime(p.paidAt)} · {p.method}</span>
 </li>
 ))}
 </ul>
 </div>
 ...
 <div>
 <p className="text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider mb-2">Historial</p>
 <ul className="space-y-2">
 ...
 return (
 <li key={e.id} className="flex gap-2 text-sm text-[var(--text-secondary)]">
 <span className="text-[var(--text-secondary)] shrink-0">{formatDateTime(e.createdAt)}</span>
 ...
 {invoice.notes && (
 <div>
 <p className="text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider mb-1">Notas</p>
