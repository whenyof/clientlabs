...
export function IssueInvoiceDialog({
 ...
 <DialogContent className="bg-zinc-900 border-[var(--border-subtle)] text-[var(--text-primary)] max-w-2xl max-h-[90vh] overflow-y-auto">
 <DialogHeader>
 ...
 {!invoice ? (
 <p className="text-[var(--text-secondary)] text-sm">No hay factura seleccionada.</p>
 ) : (
 <div className="space-y-6">
 {/* Preview: number, company, client, items, taxes, totals, due date */}
 <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)]/[0.02] p-4 space-y-4">
 <div className="grid grid-cols-2 gap-4 text-sm">
 <div>
 <p className="text-[var(--text-secondary)] uppercase tracking-wider text-xs mb-0.5">Número</p>
 <p className="text-[var(--text-primary)] font-medium">
 {invoice.number === DRAFT_NUMBER_PLACEHOLDER ? `${invoiceStatusLabel("DRAFT")} (se asignará al emitir)` : invoice.number}
 </p>
 </div>
 <div>
 <p className="text-[var(--text-secondary)] uppercase tracking-wider text-xs mb-0.5">Vencimiento</p>
 <p className="text-[var(--text-primary)] font-medium">{formatDate(invoice.dueDate)}</p>
 </div>
 </div>

 <div>
 <p className="text-[var(--text-secondary)] uppercase tracking-wider text-xs mb-1">Empresa (perfil de facturación)</p>
 {loadingCompany ? (
 <p className="text-[var(--text-secondary)] text-sm">Cargando…</p>
 ) : company ? (
 <div className="text-sm text-[var(--text-secondary)]">
 <p className="font-medium">{company.companyName}</p>
 {company.taxId && <p className="text-[var(--text-secondary)]">{company.taxId}</p>}
 {company.address && <p className="text-[var(--text-secondary)]">{company.address}</p>}
 {company.email && <p className="text-[var(--text-secondary)]">{company.email}</p>}
 </div>
 ) : (
 <p className="text-[var(--text-secondary)] text-sm">—</p>
 )}
 </div>

 <div>
 <p className="text-[var(--text-secondary)] uppercase tracking-wider text-xs mb-1">Cliente</p>
 ...
 <div>
 <p className="text-[var(--text-secondary)] uppercase tracking-wider text-xs mb-2">Líneas</p>
 ...
 <div className="pt-2 border-t border-[var(--border-subtle)] space-y-1 text-sm">
 ...
 <div className="flex justify-between text-[var(--text-primary)] font-medium pt-1">
 <span>Total</span>
 ...
 <p className="text-sm font-medium text-[var(--text-secondary)] mb-2">Completa los datos para poder emitir:</p>
 ...
 <DialogFooter className="border-t border-[var(--border-subtle)] pt-4 gap-2 sm:gap-0">
 <Button
 type="button"
 variant="outline"
 onClick={onClose}
 ...
 className="bg-[var(--accent-soft)] hover:bg-[var(--accent-soft)] text-[var(--text-primary)] border-0 disabled:opacity-50"
 >
