...
export function FileUploadDialog({
 ...
 <DialogContent className="bg-zinc-900 border-[var(--border-subtle)] max-w-lg">
 <DialogHeader>
 ...
 className="bg-zinc-800 border-zinc-600 text-[var(--text-primary)] placeholder:text-zinc-500"
 ...
 <Button type="button" onClick={handleStep1Next} className="bg-blue-600 hover:bg-blue-700 text-[var(--text-primary)]">
 Continuar
 </Button>
 </DialogFooter>
 </div>
 ...
 <p className="text-xs text-zinc-400">Documento: <span className="text-[var(--text-primary)] font-medium">{documentName || "—"}</span></p>
 </div>
 ...
 size="sm"
 className={category === value ? "bg-blue-600 text-[var(--text-primary)]" : "border-zinc-600 text-zinc-300 hover:bg-zinc-800"}
 ...
 <span className="text-sm text-[var(--text-primary)] truncate flex-1 min-w-0">{file.name}</span>
 <span className="text-xs text-zinc-500 shrink-0">{formatBytes(file.size)}</span>
 <Button
 type="button"
 variant="ghost"
 size="sm"
 className="h-7 w-7 p-0 text-zinc-400 hover:text-[var(--critical)] hover:bg-[var(--bg-card)] shrink-0"
 ...
 className="bg-green-600 hover:bg-green-700 text-[var(--text-primary)]"
 >
