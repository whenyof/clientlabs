...
function ReportingChartComponent({ data }: Props) {
    ...
    if (data.length === 0) {
        return (
            <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)] p-6 backdrop-blur min-h-[280px] flex items-center justify-center">
                <p className="text-[var(--text-secondary)] text-sm">Sin datos en el periodo seleccionado</p>
            </div>
        )
    }
    ...
    return (
        <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)] p-6 backdrop-blur">
            <h3 className="text-sm font-medium text-[var(--text-secondary)] mb-4">{chartTitle}</h3>
