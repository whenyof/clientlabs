...
function ReportingKPIsComponent({ kpis }: Props) {
 ...
 key={item.label}
 className={cn(
 "rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)] p-4 backdrop-blur",
 ...
 <p className="text-xl font-bold text-[var(--text-primary)] mt-0.5 truncate">{item.value}</p>
 </div>
 ))}
 </div>
 )
