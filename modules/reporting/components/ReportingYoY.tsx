...
function YoYBadge({ value }: { value: number | null }) {
    if (value === null) return <span className="text-[var(--text-secondary)] text-sm">—</span>
    const positive = value >= 0
    return (
        <span
            className={cn(
                "text-sm font-semibold",
                positive ? "text-[var(--accent)]" : "text-[var(--critical)]"
            )}
        >
            {value > 0 ? "+" : ""}{value}%
        </span>
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <h3 className="text-sm font-medium text-[var(--text-secondary)]">{y.title}</h3>
                <div className="flex rounded-lg bg-[var(--bg-card)] p-0.5 border border-[var(--border-subtle)]">
                    <button
                        type="button"
                        onClick={() => setPeriod("ytd")}
                        className={cn(
                            "px-3 py-1.5 rounded-md text-sm font-medium transition-colors",
                            period === "ytd" ? "bg-[var(--bg-card)] text-[white]" : "text-[var(--text-secondary)] hover:text-[white]"
                        )}
                    >
                        {y.periodYtd}
                    </button>
                    <button
                        ...
                        className={cn(
                            "px-3 py-1.5 rounded-md text-sm font-medium transition-colors",
                            period === "full" ? "bg-[var(--bg-card)] text-[white]" : "text-[var(--text-secondary)] hover:text-[white]"
                        )}
                    >
                        {y.periodFullYear}
                    </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)] p-4 backdrop-blur">
                    ...
                    <p className="text-lg font-bold text-[var(--text-primary)] mt-0.5">{yoyKpis.salesCurrent}</p>
                    <p className="text-xs text-[var(--text-secondary)] mt-0.5">
