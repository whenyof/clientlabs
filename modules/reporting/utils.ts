 ...
 * Uses unified Forecast Engine V2; projects FORECAST_AHEAD_MONTHS with 3 scenarios.
 */
export function computeRevenueForecast(
 ...
 // Adapt monthly data for the unified forecast engine
 const historySeries = monthlyHistory.map((m) => ({
 date: new Date(m.monthKey + "-01"),
 amount: m.revenue
 }))

 const forecast = runForecastV2({
 dailyRevenue: historySeries,
 minHistoryDays: 3, // Monthly needs less "points" than daily
 periodDays: historySeries.length
 })

 const monthLabels = monthlyHistory.map((m) => m.label)
 const historicalRevenue = monthlyHistory.map(m => m.revenue)

 const conservative: number[] = []
 const realistic: number[] = []
 const optimistic: number[] = []

 // Use the slope from the unified engine to project
 // Since we passed monthly data, the slope is per-month
 const lastRevenue = historicalRevenue[historicalRevenue.length - 1]

 for (let i = 1; i <= FORECAST_AHEAD_MONTHS; i++) {
 // Basic linear projection from last point using engine's slope
 const projected = Math.max(0, lastRevenue + (forecast.slope * i))
