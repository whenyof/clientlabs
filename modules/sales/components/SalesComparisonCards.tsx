...
export function SalesComparisonCards({ kpis, salesCurrent }: Props) {
 ...
 <div
 key={card.title}
 className="rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)] p-4 backdrop-blur"
 >
 <p className="text-xs text-[var(--text-secondary)] uppercase tracking-wider mb-1">{card.title}</p>
 <p className="text-lg font-semibold text-[var(--text-primary)] truncate">{card.value}</p>
 {card.sub && <p className="text-sm text-[var(--text-secondary)] truncate mt-0.5">{card.sub}</p>}
 </div>
 ))}
 </div>
 )
