...
export function SalesGoalCard({
 ...
 Gasto del mes: <span className="font-semibold text-[var(--text-primary)] tabular-nums">{formatSaleCurrency(currentMonthAmount ?? 0)}</span>
 </p>
 )}
 ...
 <span className="text-sm text-[var(--text-secondary)] tabular-nums">({progressPct}%)</span>
 </div>

 <div className="h-2 rounded-full bg-[var(--bg-card)] overflow-hidden mb-4">
 <div
 ...
 <div className="rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)]/[0.03] p-3 space-y-2">
 <div className="flex justify-between items-baseline gap-2">
 <span className="text-xs text-[var(--text-secondary)]">{isPurchases ? "Restante" : "Faltan"}</span>
 <span className="text-base font-semibold text-[var(--text-primary)] tabular-nums">
 {formatSaleCurrency(Math.round(remaining))}
 </span>
 </div>
 <div className="flex justify-between items-baseline gap-2">
 <span className="text-xs text-[var(--text-secondary)]">Necesario por día</span>
 <span className="text-base font-semibold text-[var(--text-primary)] tabular-nums">
 {formatSaleCurrency(Math.round(requiredPerDay))}
 </span>
 </div>
 <div className="flex justify-between items-baseline gap-2">
 <span className="text-xs text-[var(--text-secondary)]">Proyección actual</span>
 <span
 className={cn(
 "text-base font-semibold tabular-nums",
 projectionOnTrack ? "text-violet-400" : "text-[var(--text-secondary)]"
 )}
 >
 ...
 <DialogContent className="sm:max-w-md bg-zinc-900 border-[var(--border-subtle)] text-[var(--text-primary)]">
 <DialogHeader>
 ...
 <div>
 <Label className="text-[var(--text-secondary)]">{isPurchases ? "Presupuesto de gastos (€)" : "Objetivo de ingresos (€)"}</Label>
 <Input
 type="number"
 min={0}
 step={100}
 ...
 placeholder="25000"
 className="mt-2 bg-[var(--bg-card)] border-[var(--border-subtle)] text-[var(--text-primary)]"
 />
 </div>
 <div className="flex justify-end gap-2">
 <Button type="button" variant="ghost" onClick={() => setEditOpen(false)} className="text-[var(--text-secondary)]">
 Cancelar
 </Button>
 <Button
 type="button"
