...
const pillClass = {
 ...
 pending: "bg-[var(--bg-card)] text-[var(--text-secondary)] border-[var(--border-subtle)]",
...
export function SalesLedger({ mode = "sales", sales, onSelectSale }: Props) {
 ...
 <p className="text-sm text-[var(--text-secondary)] mt-1">{isPurchases ? "Registro de compras" : (sl?.pageSubtitle ?? "")}</p>
 </div>
 )
 }
 ...
 className="bg-[var(--bg-card)] border border-[var(--border-subtle)] rounded-xl p-4 active:scale-[0.98] transition-transform"
 >
 <div className="flex justify-between items-start mb-2">
 <div>
 <p className="font-semibold text-[var(--text-primary)] text-base">{sale.clientName}</p>
 {sale.product && <p className="text-xs text-[var(--text-secondary)] mt-0.5 max-w-[180px] truncate">{sale.product}</p>}
 </div>
 ...
 <span className="text-xs text-[var(--text-secondary)]">{formatSaleDateDisplay(sale.saleDate)}</span>
 <span
 className={cn(
 ...
 {sale.invoiceUrl ? (
 <div className="p-2 bg-[var(--bg-card)] rounded-lg text-violet-400" onClick={(e) => { e.stopPropagation(); if (sale.invoiceUrl) window.open(sale.invoiceUrl, '_blank') }}>
 ...
 <th className="text-left text-[11px] font-medium text-[var(--text-secondary)] uppercase tracking-wider py-3 px-4">
 ...
 <th className="text-right text-[11px] font-medium text-[var(--text-secondary)] uppercase tracking-wider py-3 px-4">
 {isPurchases ? "Importe" : (sl?.table?.amount ?? "Total")}
 </th>
 <th className="text-center text-[11px] font-medium text-[var(--text-secondary)] uppercase tracking-wider py-3 px-4 w-[100px]">
 ...
 key={sale.id}
 onClick={() => onSelectSale(sale)}
 className="border-b border-[var(--border-subtle)] transition-colors cursor-pointer group hover:bg-violet-500/10"
 >
 <td className="py-3.5 px-4">
 <p className="font-semibold text-[var(--text-primary)] text-sm truncate max-w-[200px] group-hover:text-violet-200">
 {sale.clientName}
 </p>
 {sale.product ? (
 <p className="text-xs text-[var(--text-secondary)] truncate max-w-[200px] mt-0.5">{sale.product}</p>
 ) : null}
 </td>
 <td className="py-3.5 px-4 text-[var(--text-secondary)] text-sm whitespace-nowrap">
 ...
 <span
 className="inline-flex items-center gap-1.5 justify-center min-w-[72px] py-1.5 px-2 rounded-lg text-[var(--text-secondary)] text-xs cursor-default"
 ...
 <ChevronRight className="h-4 w-4 text-[var(--text-secondary)] group-hover:text-violet-400 group-hover:translate-x-0.5 transition-all" />
 </td>
 </tr>
 )
 })}
