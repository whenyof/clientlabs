...
export function SalesMegaTooltip({ mode = "sales", active, payload, label, labelFormatter }: Props) {
 ...
 return (
 <div className="rounded-lg border border-[var(--border-subtle)] bg-zinc-900/95 px-4 py-3 shadow-sm backdrop-blur">
 <p className="text-base font-semibold text-[var(--text-primary)] mb-3">{displayLabel}</p>
 <dl className="space-y-1.5 text-sm">
 <div className="flex justify-between gap-4">
 <span className="text-[var(--text-secondary)]">{isPurchases ? "Gastos" : "Revenue"}</span>
 <span className="font-medium text-[var(--text-primary)] tabular-nums">{formatSaleCurrency(revenue)}</span>
 </div>
 <div className="flex justify-between gap-4">
 <span className="text-[var(--text-secondary)]">{isPurchases ? "Órdenes" : "Sales"}</span>
 <span className="font-medium text-[var(--text-primary)] tabular-nums">{salesCount}</span>
 </div>
 <div className="flex justify-between gap-4">
 <span className="text-[var(--text-secondary)]">{isPurchases ? "Coste medio" : "Avg ticket"}</span>
 <span className="font-medium text-[var(--text-primary)] tabular-nums">{formatSaleCurrency(avgTicket)}</span>
 </div>
