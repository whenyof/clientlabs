    ...
    if (!yoyMetrics.hasPreviousData) {
        return (
            <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)] p-6 backdrop-blur">
                <h3 className="text-sm font-medium text-[var(--text-secondary)] mb-2">{sl?.title ?? "Comparativa anual"}</h3>
                <p className="text-sm text-[var(--text-secondary)]">{sl?.noDataMessage ?? "No hay datos suficientes del año anterior"}</p>
            </div>
        )
    }
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <h3 className="text-sm font-medium text-[var(--text-secondary)]">{sl?.title ?? "Comparativa anual"}</h3>
                <p className="text-xs text-[var(--text-secondary)]" title={tooltipText}>{tooltipText}</p>
            </div>

            {chartData.length > 0 && (
                <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)] p-6 backdrop-blur">
                    <h4 className="text-xs font-medium text-[var(--text-secondary)] mb-4">{sl?.chartTitle ?? "Evolución comparada"}</h4>
