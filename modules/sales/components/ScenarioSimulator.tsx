...
export function ScenarioSimulator({ baseMetrics }: Props) {
 ...
 <div className="rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)]/[0.03] p-4 space-y-3">
 ...
 result.growthPct >= 0 ? "text-violet-400" : "text-[var(--critical)]"
 )}
 >
 {result.growthPct >= 0 ? "+" : ""}
 {result.growthPct}%
...
function InputRow({
 ...
 className="flex-1 min-w-0 rounded-lg bg-[var(--bg-card)] border border-[var(--border-subtle)] px-3 py-2 text-sm text-[var(--text-primary)] tabular-nums focus:outline-none focus:ring-1 focus:ring-violet-500/50"
 />
 {suffix ? (
 <span className="text-xs text-[var(--text-secondary)] shrink-0">{suffix}</span>
 ) : null}
 </div>
 </div>
 )
