...
export type DecisionEngineInput = {
 ...
 /** Series histórica para proyecciones precisas. */
...
/** 
 * Annualiza ingresos del periodo usando el motor de inteligencia.
 * Si no hay serie, recurre a extrapolación lineal básica.
 */
function annualizeRevenue(input: DecisionEngineInput): number {
 const { revenue, periodDays = 30, dailyRevenueSeries } = input
 ...
 // Annual = projected30d * 12 (more stable than daily average * 365)
 return forecast.projected30d * 12
 }
...
function decisionPricing(input: DecisionEngineInput): BusinessDecision | null {
 ...
 const annual = annualizeRevenue(input)
