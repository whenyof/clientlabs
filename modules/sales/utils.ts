...
/**
 * Advanced sales forecast from historical data. 
 * Uses Intelligence Core Forecast Engine V2 for robust projections.
 */
export function calculateSalesForecast(params: {
 ...
 // 1. Prepare historical data for the engine
 ...
 const historySeries = Array.from(historicalMap.entries())
 .sort((a, b) => a[0].localeCompare(b[0]))
 .map(([date, val]) => ({
 date: new Date(date),
 amount: val.total
 ...
 // 2. Execute Intelligence Core Forecast V2
 ...
 // 3. Extract metrics for legacy contract support
 const totalRevenue = sales.reduce((sum, s) => sum + Number(s.total), 0)
 const totalCount = sales.length
 // We use the engine's slope to determine daily rates beyond simple averages
 const dailyRevenue = historySeries.length > 0 ? totalRevenue / historySeries.length : 0
 const dailyCount = historySeries.length > 0 ? totalCount / historySeries.length : 0

 const prevRevenue = salesPrevious.reduce((sum, s) => sum + Number(s.total), 0)
 const prevCount = salesPrevious.length
 const dailyPrevRevenue = daysInPeriod > 0 ? prevRevenue / daysInPeriod : 0
 const dailyPrevCount = daysInPeriod > 0 ? prevCount / daysInPeriod : 0
 ...
 // 4. Projections Logic using Engine V2
 // We calculate end-of-month and other windows consistently
 ...
 const projEOM = (dailyRevenue * daysLeftInMonth) + (forecast.slope * daysLeftInMonth * 0.5)
 const proj30 = forecast.projected30d
 const proj90 = (dailyRevenue * 90) + (forecast.slope * 90 * 0.5)

 // Mapping to scenarios (preserving 10% margins for UI)
 const endOfMonth = scenarioFromBase(projEOM, dailyCount * daysLeftInMonth)
 const next30 = scenarioFromBase(proj30, dailyCount * 30)
 const next90 = scenarioFromBase(proj90, dailyCount * 90)

 // 5. Chart Data Construction
 const chartHistorical = historySeries.map(h => ({
 label: formatChartLabelDay(getDateKeyDay(h.date)),
 date: getDateKeyDay(h.date),
 revenue: h.amount,
 count: historicalMap.get(getDateKeyDay(h.date))?.count || 0,
 ...
 const key = getDateKeyDay(d)
 const futureRevenue = Math.max(0, dailyRevenue + (forecast.slope * i))
 chartForecast.push({
 label: formatChartLabelDay(key),
 date: key,
 revenue: 0,
 count: 0,
 forecastRevenue: futureRevenue,
 forecastCount: dailyCount,
 })
 }

 return {
 canCompute: daysInPeriod >= 7 && totalRevenue > 0,
 medias: {
 dailyRevenue: forecast.projected30d / 30, // Normalised daily for UI
 dailyCount,
 daysInPeriod,
 trendRevenuePct,
 trendCountPct,
 ...
 chartData: [...chartHistorical, ...chartForecast],
 }
}
