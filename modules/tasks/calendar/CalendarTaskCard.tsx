...
export function CalendarTaskCard({
 ...
 ? "ring-2 ring-red-500 border-[var(--critical)]"
 : hasWarning
 ? "ring-2 ring-amber-500 border-[var(--border-subtle)]"
 : ""
 ...
 {/* Resize handle top */}
 <div
 className="absolute left-0 right-0 top-0 h-2 cursor-n-resize rounded-t-lg hover:bg-[var(--bg-card)] transition-colors z-10"
 ...
 <span className="text-xs font-medium text-[var(--text-primary)] truncate block flex-1 min-w-0">{event.title}</span>
 {violations.length > 0 && (
 <AlertTriangle
 className={cn("shrink-0 w-3.5 h-3.5", hasError ? "text-[var(--critical)]" : "text-[var(--text-secondary)]")}
 aria-hidden
 />
 )}
 </div>
 ...
 <div
 className="absolute left-0 right-0 bottom-0 h-2 cursor-s-resize rounded-b-lg hover:bg-[var(--bg-card)] transition-colors z-10"
