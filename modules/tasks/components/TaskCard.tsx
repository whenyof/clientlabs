...
export const TaskCard = memo(function TaskCard({ task, onUpdate, onDelete }: TaskCardProps) {
 ...
 : "bg-zinc-900 border-[var(--border-subtle)] hover:border-[var(--border-subtle)]",
 isOverdue && !isCompleted ? "border-[var(--critical)] bg-[var(--bg-card)] hover:bg-[var(--bg-card)]" : ""
 )}
 >
 ...
 className={cn(
 "transition-all duration-300 border-[var(--border-subtle)]",
 "data-[state=checked]:bg-[var(--accent-soft)] data-[state=checked]:border-transparent data-[state=checked]:scale-110",
 ...
 isCompleted ? "text-[var(--text-secondary)] line-through decoration-white/20" : "text-[var(--text-primary)] group-hover:text-[var(--accent)]"
 )}>
 {task.title}
 </span>
 ...
 {isOverdue && (
 <Badge variant="outline" className="text-[10px] px-1.5 py-0 h-4 bg-[var(--bg-card)] border-[var(--critical)] text-[var(--critical)] animate-pulse">
 {ui.overdueBadge}
 </Badge>
 )}
 </div>

 <div className="flex items-center gap-3 text-xs text-[var(--text-secondary)] group-hover:text-[var(--text-secondary)] transition-colors">
 {/* Type */}
 <div className="flex items-center gap-1.5 bg-[var(--bg-card)] px-1.5 py-0.5 rounded-md">
 <TypeIcon className="h-3 w-3 opacity-70" />
 <span>{typeLabels[task.type] ?? task.type}</span>
 </div>

 {/* Date */}
 <div className={cn("flex items-center gap-1.5", isOverdue && "text-[var(--critical)] font-medium")}>
 ...
 {/* Context (Lead/Client) */}
 {task.clientName && (
 <div className="flex items-center gap-1 text-[var(--accent)] hover:text-[var(--accent)] transition-colors">
 ...
 <DropdownMenuItem onClick={() => setEditOpen(true)} className="text-[var(--text-primary)] hover:bg-[var(--bg-card)] cursor-pointer">
 {ui.edit}
 </DropdownMenuItem>
 <DropdownMenuItem onClick={() => setDeleteOpen(true)} className="text-[var(--critical)] hover:bg-[var(--bg-card)] cursor-pointer">
 {ui.delete}
 </DropdownMenuItem>
 </DropdownMenuContent>
 </DropdownMenu>
