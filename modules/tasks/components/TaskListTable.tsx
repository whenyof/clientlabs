...
export function TaskListTable({ className }: TaskListTableProps) {
 ...
 row.priority === "Alta" && "bg-[var(--bg-card)] text-[var(--critical)]",
 row.priority === "Media" && "bg-[var(--bg-card)] text-[var(--text-secondary)]",
 row.priority === "Baja" && "bg-[var(--bg-sidebar)] text-slate-600"
 )}
 >
 {row.priority}
 </span>
