...
/** Day/week view: height and position by duration and start time. Exported for drag- */
...
function getStatusContainerClasses(status: string): string {
 ...
 return "opacity-75 bg-[var(--bg-card)]/[0.02] border-white/[0.08] transition-all duration-200"
...
function getStatusTitleClasses(status: string): string {
 const s = (status || "").toUpperCase()
 if (s === "DONE" || s === "COMPLETED") return "line-through text-[var(--text-secondary)]"
 return ""
}
...
function getStatusBarColor(task: MissionControlCalendarItem): string {
 ...
 if (status === "DONE" || status === "COMPLETED") return "bg-[var(--accent-soft)]"
 if (overdue) return "bg-[var(--bg-card)]"
...
function getPriorityAccent(priority: string): string {
 ...
 if (p === "HIGH") return "bg-[var(--bg-card)]"
 if (p === "MEDIUM") return "bg-[var(--bg-card)]"
 return "bg-[var(--bg-sidebar)]"
}
...
const TaskCardContent = memo(function TaskCardContent({
 ...
 {isCompleted ? (
 <Check className="w-3 h-3 text-[var(--accent)] shrink-0" aria-hidden />
 ) : (
 <TypeIcon className="w-3 h-3 text-zinc-400 shrink-0" />
 )}
 <span className={cn("text-[10px] font-semibold text-[var(--text-primary)] truncate flex-1", statusTitleClass)}>
 {task.title}
 </span>
 {priorityLabel && (
 ...
 {showRisk && (
 <AlertTriangle className="w-3 h-3 text-[var(--text-secondary)] shrink-0" aria-label="Retrasada" aria-hidden />
 )}
 {showScheduleRisk && (
 <AlertCircle className="w-3 h-3 text-[var(--text-secondary)] shrink-0" aria-label={scheduleRiskTooltip} aria-hidden />
 )}
 </div>
 {meta && (
 <span className="text-[9px] text-[var(--text-secondary)] truncate block mt-0.5 opacity-70">{meta}</span>
 )}
 {timeRange && (
 <span className="text-[9px] text-[var(--text-secondary)] tabular-nums block mt-0.5">{timeRange}</span>
 )}
 </div>
 <div className="shrink-0 flex justify-end px-2 pb-1">
 <span
 className="h-4 w-4 rounded bg-[var(--bg-card)] text-[9px] flex items-center justify-center text-[var(--text-secondary)] flex-shrink-0"
 ...
 {isCompleted ? (
 <Check className="w-2.5 h-2.5 text-[var(--accent)] shrink-0" aria-hidden />
 ) : (
 <TypeIcon className="w-2.5 h-2.5 text-zinc-400 shrink-0" />
 )}
 {showRisk && <AlertTriangle className="w-2.5 h-2.5 text-[var(--text-secondary)] shrink-0" />}
 {showScheduleRisk && <AlertCircle className="w-2.5 h-2.5 text-[var(--text-secondary)] shrink-0" aria-label={scheduleRiskTooltip} />}
 </div>
 </>
 )
 }
 ...
 <span className={cn("text-xs font-semibold text-[var(--text-primary)] truncate flex-1", statusTitleClass)}>
 {task.title}
 </span>
 {priorityLabel && (
 <span className={cn("shrink-0 text-[9px] px-1.5 py-0.5 rounded", priorityBadgeClass)}>
 ...
 {showRisk && <AlertTriangle className="w-3 h-3 text-[var(--text-secondary)] shrink-0" aria-label="Retrasada" />}
 {showScheduleRisk && <AlertCircle className="w-3 h-3 text-[var(--text-secondary)] shrink-0" aria-label={scheduleRiskTooltip} />}
 </div>
 </>
 )
 }
 ...
 {meta && (
 <span className="text-[10px] text-[var(--text-secondary)] truncate block mt-0.5 opacity-70">{meta}</span>
 )}
 {timeRange && (
 <span className="text-[10px] text-[var(--text-secondary)] tabular-nums block mt-0.5">{timeRange}</span>
 )}
 </div>
 <div className="shrink-0 flex justify-end px-2 pb-1">
 <span
 className="h-5 w-5 rounded bg-[var(--bg-card)] text-[10px] flex items-center justify-center text-[var(--text-secondary)] flex-shrink-0"
 title={task.assignedTo ?? undefined}
 >
 {initials}
 </span>
 ...
 {isCompleted ? (
 <Check className="w-3.5 h-3.5 text-[var(--accent)] shrink-0" aria-hidden />
 ) : (
 <TypeIcon className="w-3.5 h-3.5 text-zinc-400 shrink-0" />
 )}
 <span className={cn("text-sm font-semibold text-[var(--text-primary)] truncate flex-1", statusTitleClass)}>
 {task.title}
 </span>
 {priorityLabel && (
 ...
 {showRisk && (
 <AlertTriangle className="w-3.5 h-3.5 text-[var(--text-secondary)] shrink-0" aria-label="Retrasada" aria-hidden />
 )}
 {showScheduleRisk && (
 <AlertCircle className="w-3.5 h-3.5 text-[var(--text-secondary)] shrink-0" aria-label={scheduleRiskTooltip} aria-hidden />
 )}
 </div>
 {meta && (
 <span className="text-xs text-[var(--text-secondary)] truncate block mt-1 opacity-70">{meta}</span>
 )}
 {timeRange && (
 <span className="text-xs text-[var(--text-secondary)] tabular-nums block mt-0.5">{timeRange}</span>
 )}
 </div>
...
const TASK_CARD_CONTAINER_CLASS =
 "rounded-xl bg-[var(--bg-card)]/[0.04] border border-[var(--border-subtle)] hover:bg-[var(--bg-card)]/[0.07] hover:border-[var(--accent)] hover:shadow-sm transition-all cursor-pointer overflow-hidden flex flex-col justify-between min-w-0"
...
export const CalendarTaskPreview = memo(function CalendarTaskPreview({
 ...
 task.kind === "REMINDER" && "border-dashed border-[var(--border-subtle)]",
 getStatusContainerClasses(task.status ?? ""),
 getAutoPriorityBorderClass(task.autoPriority ?? null),
 "shadow-sm w-56 pointer-events-none"
 )}
 >
...
const DayView = memo(function DayView({
 ...
 <div
 key={virtualRow.key}
 className="absolute left-0 right-0 flex border-b border-[var(--border-subtle)] py-0.5"
 style={{
 ...
 className="flex-1 min-h-0 rounded border border-transparent hover:border-[var(--border-subtle)] hover:bg-[var(--bg-card)]/[0.02] transition-colors"
...
const WeekView = memo(function WeekView({
 ...
 <div className="grid grid-cols-8 shrink-0 sticky top-0 z-10 border-b border-[var(--border-subtle)] bg-[var(--bg-card)] ">
 <div className="border-r border-[var(--border-subtle)] px-2 py-3 w-14 flex items-center justify-center">
 ...
 key={d.getTime()}
 className={cn(
 "border-l border-[var(--border-subtle)] px-2 py-3 text-center min-w-0",
 i % 2 === 0 ? "bg-[var(--bg-card)]/[0.02]" : "bg-transparent"
 )}
 >
 ...
 >
 <div
 className="sticky left-0 z-[1] shrink-0 border-r border-[var(--border-subtle)] bg-[var(--bg-sidebar)] relative"
 style={{ height: totalHeight }}
 >
 {virtualRows.map((virtualRow) => (
 <div
 key={virtualRow.key}
 className="absolute left-0 right-0 flex items-center border-b border-[var(--border-subtle)]"
 style={{
 height: HOUR_ROW_HEIGHT_PX,
 transform: `translateY(${virtualRow.start}px)`,
 }}
 ...
 dayKey={key}
 className={cn(
 "min-h-0 overflow-hidden border-l border-[var(--border-subtle)] first:border-l-0 relative",
 i % 2 === 0 ? "bg-[var(--bg-card)]/[0.02]" : "bg-transparent"
 )}
 style={{ height: totalHeight }}
 >
 ...
 "hover:bg-[var(--bg-card)]/[0.02]",
 isToday && "ring-1 ring-accent/30 ring-inset"
 )}
...
const MonthView = memo(function MonthView({
 ...
 <div className="grid grid-cols-7 gap-px shrink-0 border-b border-[var(--border-subtle)] bg-[var(--bg-card)]">
 {weekDayLabels.map((label) => (
 <div
 key={label}
 className="px-1 py-1.5 text-center text-xs font-medium text-zinc-500 bg-[var(--bg-card)]/[0.02]"
 >
 {label}
 </div>
 ))}
 ...
 <div
 key={wi}
 className="flex-1 min-h-0 grid grid-cols-7 gap-px border-b border-[var(--border-subtle)] bg-[var(--bg-card)]"
 >
 {week.map((d) => {
 const key = format(d, "yyyy-MM-dd")
 ...
 <div key={task.id} className={cn(TASK_CARD_CONTAINER_CLASS, task.kind === "REMINDER" && "border-dashed border-[var(--border-subtle)]", getStatusContainerClasses(task.status ?? ""), getAutoPriorityBorderClass(task.autoPriority ?? null), "shrink-0 w-full")}>
 <DraggableTaskCard
 task={task}
 sourceDayKey={key}
 onTaskClick={onTaskClick}
...
const YearView = memo(function YearView({
 ...
 className={cn(
 "rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)]/[0.02] hover:bg-[var(--bg-card)]/[0.06] hover:border-[var(--border-subtle)]",
