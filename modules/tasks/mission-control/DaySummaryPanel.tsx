...
export function DaySummaryPanel({
 ...
 <aside
 className={cn(
 "flex flex-col min-h-0 overflow-auto rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)]/[0.02]",
 className
 )}
 aria-label="Resumen del día"
 >
 <h2 className="text-xs font-semibold uppercase tracking-wider text-zinc-400 px-4 py-3 border-b border-[var(--border-subtle)] shrink-0">
 Resumen del día
 </h2>
 ...
 <div className="rounded border border-[var(--border-subtle)] bg-[var(--bg-card)]/[0.03] px-2.5 py-2">
 <span className="text-zinc-500 block">Pendientes</span>
 <span className="font-semibold tabular-nums text-[var(--text-primary)]">{summary.pendingCount}</span>
 </div>
 <div className="rounded border border-[var(--border-subtle)] bg-[var(--bg-card)]/[0.03] px-2.5 py-2">
 <span className="text-zinc-500 block">Completadas</span>
 <span className="font-semibold tabular-nums text-[var(--text-primary)]">{summary.completedCount}</span>
 </div>
 </div>
 <div>
 <h3 className="text-[11px] font-medium text-zinc-500 mb-1">Tiempo ocupado</h3>
 <p className="text-xs font-medium text-[var(--text-primary)] tabular-nums">{occupiedLabel}</p>
 </div>
 <div>
 <h3 className="text-[11px] font-medium text-zinc-500 mb-1">Huecos disponibles</h3>
 <p className="text-xs font-medium text-[var(--text-primary)] tabular-nums">{gapLabel}</p>
 </div>
 </div>
 </aside>
 )
