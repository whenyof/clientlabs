...
export function MissionControlHeader({
 ...
 <header
 className={cn(
 "w-full shrink-0 border-b border-[var(--border-subtle)]",
 className
 )}
 >
 <div className="flex items-center justify-between gap-4 px-4 py-3 min-h-[72px]">
 <div className="flex items-center gap-4 min-w-0">
 <h1 className="text-sm font-semibold text-[var(--text-primary)] truncate shrink-0">Tareas</h1>
 <div className="flex items-center gap-2">
 <Button
 variant="ghost"
 size="icon"
 className="h-9 w-9 text-zinc-400 hover:text-[var(--text-primary)] hover:bg-[var(--bg-card)] rounded-lg"
 aria-label="Anterior"
 onClick={onPrev}
 >
 <ChevronLeft className="h-4 w-4" />
 </Button>
 <span className="text-base font-semibold text-[var(--text-primary)] tabular-nums min-w-[140px] sm:min-w-[180px] text-center">
 {dateRangeLabel}
 </span>
 <Button
 variant="ghost"
 ...
 variant="ghost"
 size="sm"
 className="h-9 px-3 text-xs font-medium text-zinc-400 hover:text-[var(--text-primary)] hover:bg-[var(--bg-card)] rounded-lg"
 onClick={onToday}
 >
 Hoy
 </Button>
 ...
 <div className="flex items-center gap-1 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)]/[0.02] p-0.5 shrink-0">
 {VIEWS.map((v) => (
 <button
 key={v.id}
 type="button"
 ...
 view === v.id ? "bg-[var(--bg-card)] text-[white]" : "text-zinc-400 hover:text-[white] hover:bg-[var(--bg-card)]"
 )}
 >
 {v.label}
 </button>
 ...
 type="search"
 placeholder="Buscar"
 className="h-8 pl-8 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)]/[0.02] text-zinc-300 text-xs placeholder:text-zinc-500"
 />
 </div>
 <Select>
 <SelectTrigger className="h-8 w-[90px] rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)]/[0.02] text-zinc-300 text-[11px] hidden sm:flex">
