...
export function MissionTaskPanel({
 ...
 <SheetContent
 side="right"
 className="bg-zinc-900 border-zinc-800 text-[var(--text-primary)] w-full sm:max-w-md"
 showCloseButton={true}
 >
 <SheetHeader>
 <SheetTitle className="text-[var(--text-primary)] truncate pr-8">
 {isReminder && "🔔 "}
 {task.title}
 </SheetTitle>
 </SheetHeader>
 ...
 className="bg-zinc-800 border-zinc-600 text-[var(--text-primary)]"
 placeholder="Ej: 30"
 />
 </div>
 ...
 <SelectItem value="_none_" className="text-[var(--text-primary)] focus:bg-zinc-800">
 Sin asignar
 </SelectItem>
 {users.map((u) => (
 <SelectItem
 key={u.id}
 value={u.id}
 className="text-[var(--text-primary)] focus:bg-zinc-800"
 >
 {u.name || u.email || u.id}
 </SelectItem>
 ))}
 ...
 className="bg-zinc-800 border-zinc-600 text-[var(--text-primary)] flex-1"
 />
 <Button
 type="button"
 size="sm"
 ...
 <Button
 type="button"
 className="w-full bg-[var(--bg-card)] hover:bg-[var(--bg-card)] text-[var(--text-primary)]"
